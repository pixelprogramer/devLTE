...
$app->group('/api-erp/', function () {
    $this->group('productCategory/', function () {
        $this->post('new', function (Request $request) {
            ...
            if ($token != null) {
                ...
                if ($validacionToken == true) {
                    ...
                    $er_product_category_description = trim(strtolower((isset($parametros->er_product_category_description)) ? $parametros->er_product_category_description : null));
                    $sql = "select * from erp.er_product_categories  er_pc where er_pc.er_product_category_description like '$er_product_category_description' and er_pc.er_product_category_active = '1'";
                    $r = $connection->complexQueryNonAssociative($sql);
                    if ($r == 0) {
                        $er_product_category_state = trim(strtolower((isset($parametros->er_product_category_state)) ? $parametros->er_product_category_state : null));
                        $sql = "INSERT INTO erp.er_product_categories(
                                er_product_category_description, er_product_category_state, 
                                er_product_category_active, er_product_category_created_at)
                                VALUES ('$er_product_category_description', '$er_product_category_state', '1', '$fecha_ingreso');";
